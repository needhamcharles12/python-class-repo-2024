...
def obtaining_expenses():
    ...
    while obtaining == True:
        try:
            if (user_expenses := float(input("Please enter an expense amount or 0 to quit: $"))) == 0:
                break
            elif user_expenses < 0:
                raise ValueError ("Expense cannot be negative")
            expense_list.append(user_expenses)
        except ValueError as e:
            print(f"Invalid input: {e}. Please enter a valid expense amount or 0.")
